...
Server::Server(int_t sendBufferSize, int_t receiveBufferSize) : stopped(false), listener(0), sendBufferSize(sendBufferSize), receiveBufferSize(receiveBufferSize), cleanupTimer(*this)
{
  receiveChunkSize = receiveBufferSize < 0 ? 4096 : receiveBufferSize;
}
...
Server::Client* Server::pair(Socket& socket)
{
...
void_t Server::ClientSocket::read()
{
  ...
  recvBuffer.resize(bufferSize + server.receiveChunkSize);
  ssize_t received = Socket::recv(recvBuffer + bufferSize, server.receiveChunkSize);
  switch(received)
  {
  case -1:
    if(getLastError() == 0) // EWOULDBLOCK
